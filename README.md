...
- **Session ETF lookup** — the comparison selector opens a responsive search dialog where visitors can search by ETF name, ISIN, or Yahoo Finance symbol, choose explicit best-match or most-traded ordering, and add one of up to five session listings in a single click; the added listing is selected on Overview automatically, share-class terms such as `Acc` also trigger a broader UCITS ETF search, and partial Yahoo search failures retain successful candidates
...
- Custom-symbol validation requires usable daily price history. Raw prices remain in each listing's trading currency, so Overview defaults to a common-start indexed price comparison (each ETF starts at 100) and keeps raw listing prices as an optional view. The app does not perform FX conversion or verify tax treatment or local investor eligibility.
