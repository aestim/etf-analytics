...
- **Session ETF lookup** — visitors can search by ETF name, ISIN, or Yahoo Finance symbol and add one of up to five session listings in a single click; the added listing is selected on Overview automatically, equally relevant candidates are ordered by recent average daily volume, share-class terms such as `Acc` also trigger a broader UCITS ETF search, and an exact-symbol fallback remains available
